...
function getSettings() {
  ...
  return { shiftStart: values.shift_start || '09:00', lateGrace: Number(values.late_grace_minutes || 5), standardHours: Number(values.standard_hours || 8) };
}
...
function replyText(replyToken, text) {
  return client.replyMessage({ replyToken, messages: [{ type: 'text', text, quickReply: { items: [
...
app.get('/admin', (req, res) => {
  ...
  res.send(page('LINE 打卡管理', `<header><div><h1>出勤管理</h1><p>${month} 月報表</p></div><nav><a href="/admin/export?month=${month}">匯出 CSV</a><a href="/admin/backup">備份資料</a><a href="/admin/logout">登出</a></nav></header>
    <section class="today"><div><b>${activeEmployees.length}</b><span>正式員工</span></div><div><b>${arrived}</b><span>今日已到</span></div><div><b>${working}</b><span>目前上班中</span></div></section>
    ${pending.length ? `<section class="pending-box"><h2>待核准員工 <span>${pending.length}</span></h2>${pendingHtml}</section>` : ''}
    <section class="toolbar"><form><label>月份<input type="month" name="month" value="${month}"></label><button>查詢</button></form>
...
function page(title, body) {
  ...
  :root{font-family:Inter,"Noto Sans TC",system-ui;color:#18201d;background:#f2f5f3}*{box-sizing:border-box}body{margin:0}header{background:#063d2d;color:white;padding:28px max(5vw,20px);display:flex;justify-content:space-between;align-items:center}h1,h2,p{margin:0}header p{opacity:.75;margin-top:5px}nav{display:flex;gap:18px;flex-wrap:wrap}a{color:#06c755;text-decoration:none}header a{color:white}.today,.pending-box,.toolbar,.operations,.cards{max-width:1100px;margin:22px auto;padding:0 18px}.today{display:grid;grid-template-columns:repeat(3,1fr);gap:12px}.today div{background:white;border-radius:14px;padding:18px;box-shadow:0 3px 18px #133b2c12}.today b{display:block;font-size:28px;color:#087f5b}.today span{font-size:13px;color:#68766f}.pending-box{background:#fff7ed;border:1px solid #fed7aa;border-radius:16px;padding:18px}.pending-box h2 span{background:#c2410c;color:white;border-radius:20px;padding:2px 9px;font-size:14px}.pending-person{display:flex;justify-content:space-between;gap:15px;align-items:center;padding:13px 0;border-top:1px solid #fed7aa}.pending-person:first-of-type{margin-top:12px}.pending-person small{display:block;color:#78716c}.toolbar,.operations{display:grid;gap:12px}.toolbar form,article,.login{background:white;padding:18px;border-radius:16px;box-shadow:0 3px 18px #133b2c12}form{display:flex;gap:10px;align-items:end;flex-wrap:wrap}label{display:grid;gap:5px;font-size:13px;color:#52615b}input,select,button{font:inherit;padding:9px 11px;border:1px solid #ccd5d1;border-radius:9px;background:white}button{background:#087f5b;color:white;border:0;cursor:pointer}.operations article{padding:0}.operations h2,.operations>article>form{padding:18px}.cards{display:grid;gap:18px}article{padding:0;overflow:hidden}.employee{display:flex;justify-content:space-between;gap:16px;align-items:center;padding:20px}.employee small{color:#77827e}.employee-actions{display:flex;gap:8px;align-items:end;flex-wrap:wrap}.stats{display:grid;background:#f6faf8;text-align:center}.stats.five{grid-template-columns:repeat(5,1fr)}.stats b{padding:15px;font-size:22px}.stats small{display:block;font-weight:400;font-size:12px;color:#68766f}.warn,.status-rejected{color:#c2410c}.status-approved{color:#087f5b}.status-pending{color:#a16207}.table-wrap{overflow:auto}table{border-collapse:collapse;width:100%;min-width:760px}th,td{text-align:left;padding:11px 16px;border-top:1px solid #edf0ee;font-size:14px}td:last-child{display:flex;gap:6px;align-items:center}.in{color:#087f5b}.out{color:#2563eb}.danger{background:#fff;color:#c2410c;border:1px solid #fed7aa;padding:6px 9px}.login{max-width:380px;margin:12vh auto}.login form{margin-top:20px;display:grid}.empty{text-align:center;padding:40px;color:#66736e}@media(max-width:650px){header{align-items:flex-start;gap:15px;flex-direction:column}.employee,.pending-person{align-items:flex-start;flex-direction:column}.stats.five{grid-template-columns:repeat(2,1fr)}.stats b{font-size:18px}.today{grid-template-columns:1fr 1fr 1fr}.today b{font-size:22px}}
  </style>${body}</html>`;
}
