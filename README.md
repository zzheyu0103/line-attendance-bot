...
員工加入 LINE 官方帳號後，系統會收到 follow 事件並自動建立員工身分，不需要另外登入；之後可在聊天室輸入：
...
- LINE 請假流程提供日期選擇器，選完日期後再輸入請假原因
- 新員工加入 LINE 後會自動啟用；第一次使用時先選擇啟用分店，不需管理員逐一核准
- 已預先加入 Big City 遠東巨城、誠品生活南西、統一時代百貨台北店、拿靠早午餐林口遠雄仁愛店四個分店（可在 GPS 據點頁調整名稱、半徑或停用）
- GPS 半徑可直接在 GPS 據點頁修改並立即生效，不需要員工重新登入；預設四個新分店已調整為 500 公尺
...
資料預設儲存在 `data/attendance.db`，可用 `DATA_DIR` 指定永久磁碟路徑。系統每日 03:00 自動備份，並可在 `/admin/backups` 手動建立或下載備份。正式環境必須把 `/data` 掛載到主機的永久磁碟；Render 免費方案沒有永久磁碟，重啟或重新部署後 SQLite 資料可能消失。
